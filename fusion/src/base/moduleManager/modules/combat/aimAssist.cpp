...
{
	if (!aimassist::Enabled) return;
	if (!CommonData::SanityCheck()) return;
	if (SDK::Minecraft->IsInGuiState()) return;
	if (Menu::Open) return;

	if ((aimassist::aimKey && (!GetAsyncKeyState(VK_LBUTTON) && 1))) {
	...
	float realAimDistance = aimassist::aimDistance;
	...
	for (CommonData::PlayerData player : playerList)
	{
		if (!Java::Env->IsSameObject(thePlayer->GetInstance(), player.obj.GetInstance())) {
			...
			if ((abs(difference.x) <= aimassist::fov) && dist <= realAimDistance)
			{
				float health = player.health;
				switch (aimassist::targetPriority)
				{
				case 1:
	...
	float targetYaw = currentLookAngles.x + ((difference.x + offset) / aimassist::smooth);
	...
	if (currentLookAngles.y > anglesFoot.y || currentLookAngles.y < anglesHead.y) {
		float targetPitchFoot = currentLookAngles.y + (differenceFoot.y / aimassist::smooth);
		float targetPitchHead = currentLookAngles.y + (difference.y / aimassist::smooth);
		...
		if (diffFoot > diffHead)
		{
			...
			aimassist::data = renderPos - Vector3(0, 0.21, 0) - eLastHeadPos + (eLastHeadPos - eHeadPos) * renderPartialTicks;
		}
		else
		{
			targetPitch = targetPitchFoot;
			aimassist::data = renderPos - Vector3(0, 0.23, 0) - eLastPos + (eLastPos - ePos) * renderPartialTicks;
		}
		aimassist::pitchInfluenced = true;
		targetPitch += randomFloat(-aimassist::randomPitch, aimassist::randomPitch);
		thePlayer->SetAngles(Vector2(targetYaw, targetPitch));
	}
	else {
		aimassist::data = renderPos - eLastPos + (eLastPos - ePos) * renderPartialTicks;
		aimassist::pitchInfluenced = false;
...
{
	if (!aimassist::Enabled || !CommonData::dataUpdated) return;
	if (aimassist::fovCircle) {
	...
	if (aimassist::aimAssistFeedback) {
		if (aimassist::data.x == NAN) return;
		ImVec2 screenSize = ImGui::GetWindowSize();

		Vector2 w2s;
		if (CWorldToScreen::WorldToScreen(aimassist::data, CommonData::modelView, CommonData::projection, screenSize.x, screenSize.y, w2s))
		{
			if (w2s.x == NAN) return;

			if (aimassist::pitchInfluenced)
			{
